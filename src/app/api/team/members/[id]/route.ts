...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const [member] = await db.select().from(teamMembers).where(eq(teamMembers.id, params.id))
  ...
  const [deleted] = await db.delete(teamMembers).where(eq(teamMembers.id, params.id)).returning()
