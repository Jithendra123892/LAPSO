...
public class ErrorView extends VerticalLayout implements HasErrorParameter<Exception> {
    ...
    private void createErrorView(Exception exception) {
        ...
        H1 title = new H1("Authentication Required");
        title.getStyle()
            .set("color", "white")
            .set("margin", "20px 0 10px 0")
            .set("font-size", "2rem");

        Paragraph message = new Paragraph("Please login to access LAPSO dashboard");
        message.getStyle()
        ...
        errorCard.add(icon, title, message, loginBtn);
        add(errorCard);
    }
}